...
type
  ...
  TOnBufferDone = procedure(Sender: TComponent; var DataBuffer: Pointer;
    var DataSize: LongWord; var RepeatCount: Integer) of object;

  TAudioProcessorInitEvent = procedure(Sender: TComponent; var TotalSize: Int64)
    of object;
  TAudioProcessorFlushEvent = procedure(Sender: TComponent) of object;

  TGetParameterEvent64 = procedure(Sender: TComponent; var Param: Int64)
    of object;
  TGetParameterEvent32 = procedure(Sender: TComponent; var Param: LongWord)
    of object;

  TGetDataEvent = procedure(Sender: TComponent; var Buffer: Pointer;
    var Bytes: LongWord) of object;
  ...
  protected
    ...
    procedure GetDataInternal(var Buffer: Pointer;
      var Bytes: LongWord); override;
  ...
  published
    ...
    *)
    ...
    (* Property: OnGetBitsPerSample
      ...
      > end;
    *)
    property OnGetBitsPerSample: TGetParameterEvent32 read FOnGetBitsPerSample
      write FOnGetBitsPerSample;
    property OnGetChannels: TGetParameterEvent32 read FOnGetChannels
      write FOnGetChannels;
    (* Property: OnGetData
    ...
    *)
    ...
    property OnGetSampleRate: TGetParameterEvent32 read FOnGetSampleRate
      write FOnGetSampleRate;
    property OnGetSize: TGetParameterEvent64 read FOnGetSize write FOnGetSize;
    property OnGetTotalTime: TGetParameterEvent32 read FOnGetTotalTime
      write FOnGetTotalTime;
    (* Property: OnInit
...
{$ENDIF}
  ...
  published
    ...
    property OnPlayItemChanged: TPlayItemChangedEvent read FOnPlayItemChanged
      write FOnPlayItemChanged;
  end;
...
begin
  if (FBPS in [8, 16, 24, 32]) = False then
    FBPS := 16;
  Result := FBPS;
end;
...
begin
  ...
  FreeOptionsList(opt);
  if _device = nil then
    raise EACSException.Create('Cannot play on the "' + FCurrentDriver +
      '" device.');
end;
...
begin
  if (ci <> 0) and (ci > FFiles.Count - 1) then
    Exit;
  FCurrentItem := ci;
  if Busy then
  begin
...
begin
  ...
  if not(FInput is TAuFileIn) then
    raise EAuException.Create
      ('PlayList input should be a file-reading component.');
  Busy := True;
...
procedure TAudioPlayList.GetDataInternal(var Buffer: Pointer;
  var Bytes: Cardinal);
var
  TmpBytes: LongWord;
begin
...
// 这是主要的分析模块，原先对关键字的处理有bug，进行了修正，并增强解析能力
...
var
  ...
  State: Integer;
  // 获取属性
  function ExtractString(const aStr: AnsiString; pos: Integer): AnsiString;
  begin
    Result := '';
    if length(aStr) >= pos then
      for var i := pos to length(aStr) do
      begin
        var
        ch := aStr[i];
        if ch <> '"' then
...
// 获取时间
  function ExtractTime(const aStr: String): Integer;
  begin
    Result := -1;
    var
    s := trim(aStr);
    ...
    var
    sd := 0;
    if not trystrtoint(ss, sd) then
      Exit;
    // 分格，不是毫秒
    ss := s;
    var
    sm := 0;
    if not trystrtoint(ss, sm) then
      Exit;
    Result := mn * 60000 + sd * 1000 + (sm * 1000) div 75;
  end;
...
begin
  ...
  try
    ...
    begin
      var
      s := trim(SL[i]);
      if pos('INDEX 00', s) = 1 then
      begin
        if s = s0 then
        begin
          for var j := SL.Count - 1 downto 0 do
            if pos('INDEX 00', trim(SL[j])) = 1 then
              SL.delete(j);
          Break;
        end
        else
    ...
    while i < SL.Count do
    begin
      Ls := trim(SL.Strings[i]);
      Inc(i);
      case State of
        beforeTracks: // 解析文件头，发现轨道后进入轨道解析
          begin
            var
            P := pos('TITLE ', Ls);
            if P = 1 then
            begin
              FAlbum := ExtractString(Ls, P + 6);
              continue;
            end;
            P := pos('PERFORMER ', Ls);
            if P = 1 then
            begin
              FPerformer := ExtractString(Ls, P + 10);
              continue;
            end;
            P := pos('REM GENRE ', Ls);
            if P = 1 then
            begin
              FGenre := ExtractString(Ls, P + 10);
              continue;
            end;
            P := pos('REM DATE ', Ls);
            if P = 1 then
            begin
              FYear := ExtractString(Ls, P + 9);
              continue;
            end;
            P := pos('TRACK ', Ls);
            if P = 1 then
        ...
        NewTrack: // 轨道解析
          begin
            ...
            if P = 1 then
            begin
              with FItems[FItemsCount] do
              begin
                BeginIndex := 0;
                EndIndex := 0;
                Performer := FPerformer;
                Title := '';
              end;
              Inc(FItemsCount);
            end;
            ...
            if P = 1 then
            begin
              FItems[FItemsCount - 1].Title := ExtractString(Ls, P + 6);
            end;
            P := pos('PERFORMER ', Ls);
            if P = 1 then
            begin
              FItems[FItemsCount - 1].Performer := ExtractString(Ls, P + 10);
            end;
            P := pos('INDEX ', Ls);
            if P = 1 then
            begin
              ...
              begin
                ...
                begin
                  FItems[FItemsCount - 2].EndIndex := ms;
                end;
              end
              else if pos('01', s) = 1 then
              begin
                FItems[FItemsCount - 1].BeginIndex := ms;
                if FItemsCount > 1 then
  ...
  var
    t0, tB, tE: Cardinal;
  for var i := 0 to FItemsCount - 1 do
  begin
    tB := FItems[i].BeginIndex;
    tE := FItems[i].EndIndex;
    if i > 0 then
      t0 := FItems[i - 1].EndIndex
    else
      t0 := 0;
    if not(((tE > tB) or ((i = FItemsCount - 1) and (tE = 0))) and
      ((tB >= t0) or ((i = 0) and (t0 = 0)))) then
    begin
      FItemsCount := 0;
      Break;
    end;
...
begin
  ...
  begin
    ...
    Result := (FItems[FCurrentItem].EndIndex - FItems[FCurrentItem]
      .BeginIndex) div 1000;
end;
...
begin
  ...
  TAuSeekableStreamedInput(FInput).StartSample :=
    Round(FItems[FCurrentItem].BeginIndex / 1000 * FSR);
  if FCurrentItem = FItemsCount - 1 then
    TAuSeekableStreamedInput(FInput).EndSample := -1
  else
    TAuSeekableStreamedInput(FInput).EndSample :=
      Round(FItems[FCurrentItem].EndIndex / 1000 * FSR);
  FInput.Init;
  if TAuSeekableStreamedInput(FInput).EndSample = -1 then
    FSize := FInput.Size - TAuSeekableStreamedInput(FInput).StartSample *
      (FInput.BitsPerSample div 8) * FInput.channels +
      (FInput.BitsPerSample div 8) * FInput.channels
  else
    FSize := TAuSeekableStreamedInput(FInput).EndSample *
      (FInput.BitsPerSample div 8) * FInput.channels - TAuSeekableStreamedInput
      (FInput).StartSample * (FInput.BitsPerSample div 8) * FInput.channels +
      (FInput.BitsPerSample div 8) * FInput.channels;
end;
...
begin
  ...
  if not LibGainLoaded then
    raise EAuException.Create(Format('Could not load the %s library.',
      [LibGainPath]));
...
begin
  ...
  begin
    ...
    Busy := False;
    raise EAuException.Create
      (Format('Failed to set up gain analysis. Possible cause: sample rate %d is not supported.',
      [SR]));
  end;
  FTitleGain := 0;
  FTitlePeak := 0;
...
procedure TGainAnalysis.GetDataInternal(var Buffer: Pointer;
  var Bytes: Cardinal);
var
  i: Integer;
...
begin
  ...
  case FSampleSize of
    1:
      ByteToSingle(PBuffer8(Buffer), @InBuffer[0], SamplesRead);
    2:
      SmallIntToSingle(PBuffer16(Buffer), @InBuffer[0], SamplesRead);
    3:
      Int24ToSingle(PBuffer8(Buffer), @InBuffer[0], SamplesRead);
    4:
      Int32ToSingle(PBuffer32(Buffer), @InBuffer[0], SamplesRead);
  end;
  ...
  if AnalyzeSamples(@LBuffer[0], @RBuffer[0], FramesRead, FInput.channels) = GAIN_ANALYSIS_ERROR
  then
...
begin
  Result := AnsiStrLower
    (PAnsiChar(AnsiString(ExtractFileExt(String(FFileName)))));
end;

procedure TTagEditor.SetFileName(const aFileName: AnsiString);
const
  MaxAllowedExtIndex = 5;
  AllowedExtensions: array [0 .. 5] of AnsiString = ('.mp3', '.wma', '.m4a',
    '.flac', '.wv', '.ogg');
var
  _File: PTagLib_File;
  _Tag: PTagLib_Tag;
